...
export default function AttendancePage() {
    ...
    const handleStatusChange = async (playerId: number, status: 'present' | 'absent' | 'excused' | 'late') => {
        ...
        // if (isPast) return; // Block past? User requirement: "block attendance of previous days". 
        // But also "if nothing marked in previous days, mark as absent".
        // This suggests we CANNOT change past days manually? Or maybe we can only set "Absent" if it's null?
        // Let's assume strict block for now as per "block attendance of previous days".
        if (isPast) {
                                    ...
                                    <tr key={player.id} className="hover:bg-gray-900 transition-colors">
                                        ...
                                        <td className="p-4 text-center">
                                            <button
                                                ...
                                                className={`w-4 h-4 rounded-full border border-green-500 ${(currentStatus === 'present' && !(!record?.status && isPast)) ? 'bg-green-500' : 'hover:bg-green-500/50'} disabled:opacity-30 disabled:cursor-not-allowed transition-all`}
                                            />
                                        </td>
                                        <td className="p-4 text-center">
                                            <button
                                                ...
                                                className={`w-4 h-4 rounded-full border border-red-500 ${currentStatus === 'absent' ? 'bg-red-500' : 'hover:bg-red-500/50'} disabled:opacity-30 disabled:cursor-not-allowed transition-all`}
                                            />
                                        </td>
                                        <td className="p-4 text-center">
                                            <button
                                                ...
                                                className={`w-4 h-4 rounded-full border border-yellow-500 ${currentStatus === 'late' ? 'bg-yellow-500' : 'hover:bg-yellow-500/50'} disabled:opacity-30 disabled:cursor-not-allowed transition-all`}
                                            />
                                        </td>
                                        <td className="p-4 text-center">
                                            <button
                                                ...
                                                className={`w-4 h-4 rounded-full border border-blue-500 ${currentStatus === 'excused' ? 'bg-blue-500' : 'hover:bg-blue-500/50'} disabled:opacity-30 disabled:cursor-not-allowed transition-all`}
                                            />
                                        </td>
                                        <td className="p-4 text-center">
                                            {saving === player.id ? (
